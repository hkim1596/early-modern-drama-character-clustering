...
def write_outputs(out_rows, rows, fieldnames_in, n_crowd, tcp_key, output: Path):
    # NOTE: ALL editions are kept through table-building and embedding —
    # edition deduplication happens at CLUSTERING time only (stage 04,
    # config.DEDUPE_EDITIONS), which records its decisions in
    # data/edition_dedup__<name>.csv. Alternative editions therefore stay
    # available for edition-comparison work.
